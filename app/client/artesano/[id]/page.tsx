...
export default function ArtesanoPage() {
  ...
  const nombreCompleto = [artesano.nombre, artesano.apellido].filter(Boolean).join(" ");
  const comunidad = artesano.comunidad?.trim() ?? "";
  const tipo = artesano.tipo?.trim() ?? "Maestro Artesano";
  const historia =
    artesano.historia?.trim() ||
    artesano.biografia?.trim() ||
    "Este maestro artesano ha dedicado su vida a preservar técnicas transmitidas de generación en generación.";
  const bioCorta =
    artesano.biografia?.trim() ||
    "Conoce el oficio, la comunidad y las piezas que mantienen viva la tradición artesanal.";
  ...
  const galeriaCurada = [
    ...(artesano.foto_perfil
      ? [
        {
          ...
          descripcion: `Retrato de ${nombreCompleto || artesano.nombre}`,
          tipo: "perfil",
          origen: "artesano",
        },
      ]
      : []),
    ...galeria,
  ].filter((imagen, index, lista) => {
    const url = imagen?.url;
    return Boolean(url) && lista.findIndex((item) => item?.url === url) === index;
  });
      ...
      <section className="relative bg-[#2C1810] text-[#F5EFE6] overflow-hidden rounded-b-[2.5rem] shadow-xl mb-6 pb-4 md:pb-6">
        ...
        <div className="relative z-20 max-w-7xl mx-auto px-6 pt-6 pb-4 lg:pt-10 lg:pb-8">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6 md:mb-8">
            <button
              onClick={() => router.back()}
              className="group relative z-30 flex items-center gap-2 bg-white/10 backdrop-blur-md border border-white/20 text-[#F5EFE6] px-4 py-2 md:px-5 md:py-2.5 rounded-full hover:bg-white hover:text-[#2C1810] transition-all duration-300 w-fit shadow-sm"
            >
              ...
              <span className="font-medium text-xs md:text-sm tracking-wide">Volver</span>
            </button>

            <nav className="text-[10px] md:text-xs text-[#E6D6C5] flex gap-2">
          ...
          <div className="grid lg:grid-cols-12 gap-8 items-center">
            <div className="lg:col-span-7">
              <p className="inline-flex items-center gap-2 bg-white/10 backdrop-blur-md px-3 py-1.5 rounded-full text-[10px] sm:text-xs font-bold tracking-[0.2em] uppercase text-[#E6D6C5] mb-4 border border-white/10 shadow-sm">
                ...
                {[tipo, comunidad].filter(Boolean).join(" · ") || "Maestro Artesano"}
              </p>
              <h1 className="text-4xl sm:text-5xl md:text-6xl lg:text-7xl font-bold leading-tight mb-6 tracking-tight text-white drop-shadow-md">
                {artesano.nombre}
                {artesano.apellido && (
                  <span className="block text-[#C4A882] italic font-light">{artesano.apellido}</span>
                )}
              </h1>
        ...
        <div className="max-w-7xl mx-auto px-6 grid lg:grid-cols-12 gap-16 items-start">
          ...
          <div className="lg:col-span-6 prose prose-lg">
            ...
            <h2 className="text-3xl md:text-5xl font-bold text-[#2C1810] mb-8 leading-tight text-pretty">
              {comunidad
                ? `Un oficio vivo que nace orgullosamente en ${comunidad}.`
            ...
            <p className="text-[#5C4A3A] leading-loose text-lg font-medium text-pretty opacity-90">
              <span className="float-left text-7xl font-bold text-[#E8D8C4] leading-[0.8] mr-3 mt-1">
                {historia ? historia.charAt(0) : (artesano.biografia ? artesano.biografia.charAt(0) : "E")}
              </span>
              {historia ? historia.substring(1) : (artesano.biografia ? artesano.biografia.substring(1) : "ste maestro artesano ha dedicado su vida a preservar técnicas transmitidas de generación en generación.")}
            </p>
            ...
            {artesano.ubicacion && (
              <div className="mt-8 flex items-center gap-3 text-sm text-[#A08070] bg-white p-4 rounded-xl border border-[#F0E8DC] w-fit shadow-sm">
                ...
                <span className="font-semibold text-[#5C4A3A]">Taller ubicado en:</span> {artesano.ubicacion}
              </div>
            )}
          </div>
          ...
          <div className="lg:col-span-4 space-y-6">
            {comunidad && (
              <div className="relative rounded-[2rem] border border-[#E8D8C4] bg-gradient-to-br from-[#FCF8F3] to-[#F5EFE6] p-8 shadow-sm overflow-hidden group">
                ...
                <p className="text-2xl font-bold text-[#2C1810] mb-4 z-10 relative">{comunidad}</p>
            ...
            <div className="grid grid-cols-2 gap-4">
              {[
                ...
                { val: tipo || "Maestro", label: "Especialidad", icon: "✨" },
              ].map((s) => (
        ...
        <section className="py-20 bg-white">
          <div className="max-w-7xl mx-auto px-6">
            <div className="text-center mb-10">
              <p className="text-xs tracking-widest uppercase text-[#A08070] mb-2">Galería visual</p>
